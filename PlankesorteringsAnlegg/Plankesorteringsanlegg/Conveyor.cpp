...
Wheel::Wheel(b2Vec2 p,float32 r,int cmBits,b2World* world){
	m_speed=0;
	b2BodyDef bd;
	bd.position=p;
...
Beam::Beam(b2Vec2 p1,b2Vec2 p2,float32 width,int cmBits,b2World* world){
	//center
	b2Vec2 m = p2+p1;
	m*=0.5f;
...
ConveyorBelt::ConveyorBelt(b2Vec2 p1,b2Vec2 p2,float32 beamThickness,float32 beltThickness,int N,int M,t_medbringerType medbringerType,int sensorPos,float32 sensorAdjustment,int cmBits,b2World* world){
...
float32 ConveyorBelt::getAngleAt(b2Vec2 p){
	float32 angle;
	b2Vec2 u=m_p2-m_p1;
...
ConveyorBeltShield::ConveyorBeltShield(b2Vec2 p1,b2Vec2 p2,float32 beamThickness,float32 beltThickness,int cmBits,b2World* world){
	m_p1=p1;
	m_p2=p2;
...
Conveyor::Conveyor(b2Vec2 p1, b2Vec2 p2, float32 beamThickness,float32 beltThickness,int N,int M,t_medbringerType medbringerType,int sensorPos,float32 sensorAdjustment,int cmBits, b2World* world){
	m_speed=0;
	m_bPaused=false;
	m_beamThickness=beamThickness;
	m_beltThickness=beltThickness;
...
void Conveyor::setSpeed(float32 speed){
...
ContactSensor* Conveyor::getSensor(){
	return m_conveyorBelt->getSensor();
}

bool Conveyor::isSensorActivated(){
